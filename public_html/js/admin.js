...
function set_upload_button(sel, template, dropzone, appendzone, field) {

    var $btn = $('#save_photos');
    var $template = template ? $(template) : $('#photo_template');
    var $drop_zone = dropzone ? $(dropzone) : $('#dropzone');
    var $append_zone = appendzone ? $(appendzone) : $('#appendzone');
    var field = field || '#reviews-img_name';
    ...
    $(sel).fileupload({
        ...
        success: function(resp) {
            ...
            var $photo = $($template.html());

            $('img', $photo).attr('src', resp.img_small);
            if ($append_zone.hasClass('review-img')) {
                $(field, $photo).val(resp.img_name);
...
function remove_uploaded_photo(obj) {
    ...
    if ($(obj).closest('[id^=appendzone] .photo-uploaded').length == 0) {
